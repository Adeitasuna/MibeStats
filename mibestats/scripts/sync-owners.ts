...
async function main() {
  ...
  if (ownerMap.size > 0) {
    ...
    for (let i = 0; i < entries.length; i += BATCH) {
      ...
      const cases = b.map(([id, addr]) =>
        Prisma.sql`WHEN ${id} THEN ${addr}`
      )
      const ids = b.map(([id]) => id)
      await prisma.$executeRaw`
        UPDATE tokens SET owner_address = CASE token_id ${Prisma.join(cases, ' ')} END
  ...
  if (transferCounts.size > 0) {
    ...
    for (let i = 0; i < entries.length; i += BATCH) {
      ...
      const ids = b.map(([id]) => id)
      await prisma.$executeRaw`
        UPDATE tokens SET transfer_count = transfer_count + CASE token_id ${Prisma.join(cases, ' ')} END
        WHERE token_id IN (${Prisma.join(ids)})
      `
    }
